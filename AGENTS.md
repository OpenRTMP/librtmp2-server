...
- **Dependency source:** `librtmp2` resolves from GitHub (pinned rev for 0.7.0 APIs) until crates.io publishes `0.7.0`. It does **not** use the sibling `../librtmp2` checkout unless you add a `[patch]` override locally (see comment in `Cargo.toml`).
